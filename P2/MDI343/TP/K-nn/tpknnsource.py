...
class ErrorCurve(object):
    ...
    def plot(self, marker='o', maketitle=True, **kwargs):
        label = "number of training points : %d" % len(self.y)
        plt.plot(self.k_range, self.errors, marker=marker, label=label, **kwargs)
        plt.xlabel("K")
...
class LOOCurve(object):
    ...
    def plot(self, marker='o', maketitle=True, **kwargs):
        label = "number of training points : %d" % (len(self.y) - 1)
        plt.plot(self.k_range, self.cv_scores, marker=marker, label=label, **kwargs)
        plt.xlabel("K")
